...
def main():
    ...
    data_path = './data'
    solver = CaptioningSolver(model, data_path, val_data, n_epochs=1, batch_size=128,
                update_rule='adam', learning_rate=0.001, print_every=100, save_every=1,
                pretrained_model=None, model_path='model/lstm/',
                test_model='model/lstm/model-1', print_bleu=True, log_path='log/', V=len(word_to_idx))
    solver.train()
