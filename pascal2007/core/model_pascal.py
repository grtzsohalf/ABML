...
class CaptionGenerator(object):
    ...
    def _word_embedding(self, x, reuse=False):
        '''
    ...
    def build_model(self):
        ...
        x = tf.zeros([batch_size, self.V], tf.float32)
        x += tf.to_float(tf.one_hot(start_ind, self.V, on_value=1))
        emb = self._word_embedding(x=x)
        ...
        for t in range(self.T):
            ...
            with tf.variable_scope('lstm', reuse=(t!=0)):
                _, (c, h) = lstm_cell(inputs=tf.concat(1, [emb, context]), state=[c, h])

            logits = self._decode_lstm(emb, h, context, dropout=self.dropout, reuse=(t!=0))
            ...
            # next label
            x = tf.to_float(tf.one_hot(next_ind, self.V, on_value=1))
            emb = self._word_embedding(x=x, reuse=True)
    ...
    def build_sampler(self, max_len=7):
        ...
        for t in range(max_len):
            if t == 0:
                start_ind = tf.ones([batch_size], tf.int32)
                x = tf.zeros([self.V], tf.float32) + tf.to_float(tf.one_hot(start_ind, self.V, on_value=1))
                emb = self._word_embedding(x=x)
            else:
                x = tf.to_float(tf.one_hot(sampled_word, self.V, on_value=1))
                emb = self._word_embedding(x=x, reuse=True)
            ...
            logits = self._decode_lstm(emb, h, context, reuse=(t!=0))
            logits += mask
    ...
    def init_sampler(self):
        ...
        logits = self._decode_lstm(emb, h, context)
        sampled_word = tf.argmax(logits, 1)
        # sampled_word_list.append(sampled_word)
    ...
    def word_sampler(self):
        ...
        logits = self._decode_lstm(emb, h, context, reuse=True)
        sampled_word = tf.argmax(logits, 1)
        # sampled_word_list.append(sampled_word)
