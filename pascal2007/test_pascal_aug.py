from core.solver_pascal_aug import CaptioningSolver
from core.model_pascal_aug import CaptionGenerator
from core.utils_pascal_aug import *
import os
import sys
os.environ['CUDA_VISIBLE_DEVICES']='1'

modelname = sys.argv[1]
filename = sys.argv[2]
thres = sys.argv[3]
print '#########################'
print 'model = ' + modelname
print 'thres = ' + thres
print '#########################'

def main():
    word_to_idx = load_word_to_idx(data_path='./pascaldata_aug', split='train')
    '''
    word2idx = load_word2idx(data_path='./pascaldata_aug', split='train')
    idx_to_word = {i+3: w for w, i in word2idx.iteritems()}
    idx_to_word[0] = '<NULL>'
    idx_to_word[1] = '<START>'
    idx_to_word[2] = '<END>'
    '''
    val_data = load_coco_data(data_path='./pascaldata_aug', split='val')
    # test_data = load_coco_data(data_path='./pascaldata_aug', split='test')
    model = CaptionGenerator(word_to_idx, dim_feature=[196, 512], dim_embed=16,
                            dim_hidden=1024, n_time_step=16, prev2out=True,
                            ctx2out=True, alpha_c=1.0, selector=True, dropout=True)
    data_path = './pascaldata_aug'
    solver = CaptioningSolver(model, data_path, n_epochs=200, batch_size=1,
                update_rule='adam', learning_rate=0.0005, print_every=100, save_every=1,
                pretrained_model=None, model_path='model/lstm/',
                test_model=('model/lstm/%s' %modelname), print_bleu=True, log_path='log/',
                V=len(word_to_idx))
    solver.test(val_data, split='val', filename=filename, attention_visualization=False,\
                thres=float(thres))

if __name__ == "__main__":
    main()
