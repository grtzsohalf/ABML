...
class CaptionGenerator(object):
    ...
    def _decode_lstm(self, h, context, y, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            ...
            if self.prev2out:
                h_logits = tf.concat(1, [h_logits, y])
    ...
    def build_model(self):
        ...
        for t in range(5):
            ...
            with tf.variable_scope('lstm', reuse=(t!=0)):
                _, (c, h) = lstm_cell(inputs=tf.concat(1, [context, y]), state=[c, h])

            logits = self._decode_lstm(h, context, y, dropout=self.dropout, reuse=(t!=0))
            logits = logits[:, 3:]
            y = logits
    ...
    def init_sampler(self):
        ...
        logits = self._decode_lstm(h, context, y)
        y = logits[:, 3:]
        return logits, c, h, alpha, y
    ...
    def word_sampler(self):
        ...
        logits = self._decode_lstm(h, context, y, reuse=True)
        y = logits[:, 3:]
        return logits, c, h, alpha, y
