        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                if (e+1) % self.save_every == 0:
                    saver.save(sess, os.path.join(self.model_path, 'mscoco_recursive_concat'), global_step=e+1)
                    print "mscoco_recursive_concat-%s saved." %(e+1)
    ...
    def test(self, data, split='train', attention_visualization=True, save_sampled_captions=True,\
        ...
        with tf.Session(config=config) as sess:
            ...
            for thres_iter in range(1):
                ...
                self.evaluate(all_candidate_y, 0.3, 'cocodata/val/result_%s_iter.txt' % filename)
