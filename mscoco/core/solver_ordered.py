# use initial prediction at t=0, 
# for t > 0, feed hard prediction(p) and soft prediciton(y) got from time t-1 into LSTM
...
class CaptioningSolver(object):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            part_num = 20
            ordered_list = []
            with open('label_count.txt', 'r') as f:
                lines = f.readlines()
            for idx in lines:
            ...
            for e in range(self.n_epochs):
                ...
                for part in range(part_num):
                    ...
                    # 17 is n_time_step+1
                    ordered_captions = np.zeros((n_examples, 17), dtype=np.float32)
                    for n, cap in enumerate(captions):
                        ordered_captions[n][0] = 1
                        count = 1
                        for idx in ordered_list:
                            if idx in cap:
                                ordered_captions[n][count] = idx
                                count += 1
                    ...
                    ordered_captions = ordered_captions[rand_idxs]
                    groundtruth = groundtruth[rand_idxs]
                    image_idxs = image_idxs[rand_idxs]

                    for i in range(n_iters_per_part):
                        captions_batch = ordered_captions[i*self.batch_size:(i+1)*self.batch_size]
                ...
                if (e+1) % self.save_every == 0:
                    saver.save(sess, os.path.join(self.model_path, 'mscoco_ordered'), global_step=e+1)
                    print "mscoco_ordered-%s saved." %(e+1)
        ...
        with tf.Session(config=config) as sess:
            ...
            for part in range(part_num):
                if split == 'val':
                    ...
                    print "part: ", part
                data = load_coco_data(data_path='./cocodata', split=split, load_init_pred=True, part=str(part))
                ...
                for i in tqdm(range(num_iter)):
                    ...
                    for t in range(MAX_LEN): # time step
                        ...
                        for j in range(K):
                            if t == 0:
                                ...
                                probsNumpy = probsNumpy.reshape(self.V)
                            probsNumpy = self.softmax(probsNumpy[3:])
                            probs = []
                    ...
                    alphas = paths_info[0][3] # (T, N=1, L)
                    alpha_list = np.transpose(alphas, (1, 0, 2))     # (N=1, T, L)
            ...
            if attention_visualization:
                ...
                for idx in range(num_samples):
                    r = reference[idx][0][:-2].split()
                    for ww in range(len(r)):
                        r[ww] = str(self.model.idx_to_word[int(r[ww])+3])
                    groundtruth.append(' '.join(r))
                count = 0
                for n in range(num_samples):
                    ...
                    sample_file.write(all_decoded[n]+'\n')
                    # Plot original image
