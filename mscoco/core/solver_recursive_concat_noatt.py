...
class CaptioningSolver(object):
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                if (e+1) % self.save_every == 0:
                    saver.save(sess, os.path.join(self.model_path, 'mscoco_recursive_concat_noatt'), global_step=e+1)
                    print "mscoco_recursive_concat_noatt-%s saved." %(e+1)
    ...
    def test(self, split='train', attention_visualization=True, save_sampled_captions=True,\
        ...
        config = tf.ConfigProto(allow_soft_placement=True)
        config.gpu_options.allow_growth = True
        probabilities_start, c_start, h_start, y_start, p_start = self.model.init_sampler()
        probabilities, c, h, y, p = self.model.word_sampler()
        with tf.Session(config=config) as sess:
                            ...
                            for j in range(K):
                                if t == 0:
                                    ...
                                    feed_dict = { self.model.features: features_batch,
                                                    ...
                                                    self.model.p: p_run}
                                    probsNumpy, c_run, h_run, y_run, p_run = \
                                    sess.run([probabilities, c, h, y, p], feed_dict)
                                    probsNumpy = probsNumpy.reshape(self.V)
                                ...
                                for k in range(len(probs)):
                                    ...
                                    dic[idx] = (candidate_path, c_run , h_run, samp_run, y_run, p_run)
                            count = 0
                            newPaths_info = []
                            newPathProbs = []
