...
# =========================================================================================
# This model is for experiment of ordering training
...
class CaptionGenerator(object):
    ...
    def _decode_lstm(self, h, context, y, p, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            ...
            w_out = tf.get_variable('w_out', [self.V + (self.V-3) + (self.V-3), self.V], initializer=self.weight_initializer)
            ...
            if self.prev2out:
                h_logits = tf.concat(1, [h_logits, y, p])
    ...
    def build_model(self):
        ...
        for t in range(5):
            ...
            with tf.variable_scope('lstm', reuse=(t!=0)):
                _, (c, h) = lstm_cell(inputs=tf.concat(1, [context, y, p]), state=[c, h])

            logits = self._decode_lstm(h, context, y, p, dropout=self.dropout, reuse=(t!=0))
            # for convenience, keep logits dim=V when compare with caption
    ...
    def init_sampler(self):
        ...
        logits = self._decode_lstm(h, context, y, p)
        y = logits[:, 3:]
        return logits, c, h, alpha, y, p
    ...
    def word_sampler(self):
        ...
        logits = self._decode_lstm(h, context, y, p, reuse=True)
        y = logits[:, 3:]
        return logits, c, h, alpha, y, p
