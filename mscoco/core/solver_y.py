...
# for t > 0, feed soft prediction(y) got from time t-1 into LSTM
...
class CaptioningSolver(object):
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                if (e+1) % self.save_every == 0:
                    saver.save(sess, os.path.join(self.model_path, 'mscoco_y'), global_step=e+1)
                    print "mscoco_y-%s saved." %(e+1)
    ...
    def test(self, data, split='train', attention_visualization=True, save_sampled_captions=True,\
        ...
        config = tf.ConfigProto(allow_soft_placement=True)
        config.gpu_options.allow_growth = True
        probabilities_start, c_start, h_start, alpha_start, y_start = self.model.init_sampler()
        probabilities, c, h, alpha, y = self.model.word_sampler()
        with tf.Session(config=config) as sess:
            ...
            for thres_iter in range(1):
                ...
                for i in range(num_iter):
                        ...
                        for j in range(K):
                            if t == 0:
                                ...
                                feed_dict = { self.model.features: features_batch,
                                                ...
                                                self.model.y: y_run }
                                probsNumpy, c_run, h_run, alpha_run, y_run = \
                                sess.run([probabilities, c, h, alpha, y], feed_dict)
                                probsNumpy = probsNumpy.reshape(self.V)
                            ...
                            for k in range(len(probs)):
                                ...
                                dic[idx] = (candidate_path, c_run , h_run, alphas, samp_run, y_run)
                        count = 0
                        newPaths_info = []
                        newPathProbs = []
                    ...
                    # all_candidate_y.append(candidate_y)
                ...
                # self.evaluate(all_candidate_y, 0.3, 'cocodata/val/result-%s.txt' % filename)
