...
class CaptionGenerator(object):
    ...
    def _decode_lstm(self, h, y, p, dropout=False, reuse=False):
    ...
    def build_model(self):
        ...
        for t in range(5):
            ...
            with tf.variable_scope('lstm', reuse=(t!=0)):
                _, (c, h) = lstm_cell(inputs=tf.concat(1, [y, p]), state=[c, h])

            logits = self._decode_lstm(h, y, p, dropout=self.dropout, reuse=(t!=0))
            logits = logits[:, 3:]
            y = logits
    ...
    def build_sampler(self, max_len=15):
        ...
        for t in range(max_len):
            ...
            '''
            with tf.variable_scope('lstm', reuse=(t!=0)):
                _, (c, h) = lstm_cell(inputs=tf.concat(1, [init_pred]), state=[c, h])

            logits = self._decode_lstm(h, init_pred, reuse=(t!=0))
            init_pred = logits[:, 3:]
            logits += mask
    ...
    def init_sampler(self):
        ...
        logits = self._decode_lstm(h, y, p)
        y = logits[:, 3:]
        return logits, c, h, y, p
    ...
    def word_sampler(self):
        ...
        logits = self._decode_lstm(h, y, p, reuse=True)
        y = logits[:, 3:]
        return logits, c, h, y, p
