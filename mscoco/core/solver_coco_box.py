        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                if (e+1) % self.save_every == 0:
                    saver.save(sess, os.path.join(self.model_path, 'mscoco_lr0.001'), global_step=e+1)
                    print "mscoco_lr0.001-%s saved." %(e+1)
        ...
        with tf.Session(config=config) as sess:
            ...
            for thres_iter in range(1):
                ...
                for i in range(num_iter):
                    if i % 5 == 0:
                        merge_list = []
                    for label in all_sam_cap[i]:
                        if label not in merge_list:
                            merge_list += [label]
                    if i % 5 == 4:
                        merge_cap += [merge_list]
                all_decoded = decode_py_captions(merge_cap, self.model.idx_to_word)
                save_pickle(merge_cap, "./cocodata_box/%s/%s.candidate.captions_%s_%s.pkl" % \
            ...
            if attention_visualization:
                ...
                reference = load_pickle('./cocodata_box/val/val.references.pkl')
