...
def load_word_to_idx(data_path='./cocodata_box', split='train'):
...
def load_word2idx(data_path='./cocodata_box', split='train'):
    data_path = os.path.join(data_path, split)
    start_t = time.time()
    if split == 'train':
...
def load_coco_data(data_path='./cocodata_box', split='train', part=''):
    data_path = os.path.join(data_path, split)
    start_t = time.time()
    data = {}
    if split == 'train':
