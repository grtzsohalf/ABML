...
def main():
    ...
    val_data = _process_caption_data(caption_file='/home/jason6582/sfyc/mscoco/annotations/val_aug.json',
    ...
    for i in range(part_num-1):
        save_pickle(test_data[test_cutoff[i]:test_cutoff[i+1]],
                'cocodata/test/test.annotations_%s.pkl' % str(i))
    save_pickle(test_data[test_cutoff[part_num-1]:], \
                'cocodata/test/test.annotations_%s.pkl' %(part_num-1))
    split = 'test'
    for part in range(part_num):
        annotations = load_pickle('./cocodata/%s/%s.annotations_%s.pkl' % (split, split, str(part)))
        captions = _build_caption_vector(annotations=annotations, word_to_idx=word_to_idx, max_length=max_length)
