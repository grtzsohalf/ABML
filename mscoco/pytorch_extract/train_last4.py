...
# create a resnet module which doesn't have last three layer, output dim = (196, 1024)
...
optimizer = torch.optim.SGD(resnet.fc.parameters(),
                            0.1, # Learning rate
                            momentum=0.9,
                            weight_decay=1e-4)
...
for epoch in range(training_epoch):
    ...
    if (epoch+1) % test_every == 0:
        ...
        print 'overall F1: ', o_f1
        print 'classwise F1: ', c_f1

    if (epoch+1) % save_every == 0:
        filename = 'model/resnet_last4_%s.pth.tar' % str(epoch+1)
